...
ggplot(data = covid_survey_summary_stats, aes(x=mean, y = explanatory_value, xmin= low, xmax = high, color = explanatory_value)) +
  geom_errorbarh(position = position_dodge(width = 1)) +
  geom_point() +
  facet_grid(.~response, 
             labeller = labeller(response = c("resp_concern_safety" = "I am concerned 
about the safety 
and side effects of 
the vaccine", 
...
community members"
  ...
  theme(legend.position = "none",
        ...
        strip.text.x.top = element_text(size = 12, hjust = 0.5, face = "bold")
        ) 
...
```
   ...
   scale_y_discrete(labels = c("exp_race" = "Race", 
...
vaccine"),
  ...
  geom_text(aes(label = explanatory_value), 
            size = 3, color = "black", position=position_jitter(width=1,height=1)) +
facet_grid(.~response, 
             labeller = labeller(response = c("resp_concern_safety" = "I am concerned 
about the safety 
and side effects of 
...
ggplot(data = covid_survey_summary_stats, aes(x=mean, y = explanatory, xmin= low, xmax = high, color = explanatory_value)) +
