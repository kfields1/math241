...
libs <- c('tidyverse','knitr','viridis','mdsr', 'mosaicData', 'macleish','babynames')
for(l in libs){
...
count_name <- function(input){
  if(!is.character(input)) {
    stop('Unfortunately this function only works for character inputs.\n',
         'You have provided an object of class:', class(input))
    }
  ...
  if (nrow(result) == 0) {
    stop('Unfortunately there are no matches')
  }
  
  return(data.frame(result))
}

count_name(input = "Ezekiel")
```
