...
```{r, eval = F}
census_api_key("c0e52b585ba085eacc27c70fd3b1995fd52c2dd0", install = TRUE, overwrite=TRUE)
```
...
or_county %>% 
  mutate(prop_white = white_popE/total_popE *100) %>% 
  st_as_sf() %>% 
  ggplot() +
    geom_sf(aes(fill = prop_white)) +
    scale_fill_viridis_c() +
    labs(fill = "Percentage of Population that is White") +
    theme_void()
...
#merging census data and spatial data
ca_county <- left_join(ca, ca_sf, by = c("GEOID" = "GEOID"))

#visualizing median house values across california
ca_county %>%
  st_as_sf() %>%
  ggplot() +
...
#data wrangling - just bay area
bayarea <- ca_county %>% 
  filter(NAME.x == "Alameda County, California" | 
           NAME.x == "Napa County, California" |
           NAME.x == "Santa Clara County, California"|
           NAME.x == "Contra Costa County, California"|
           NAME.x == "San Francisco County, California"|
           NAME.x == "Solano County, California"|
           NAME.x == "Marin County, California"|
           NAME.x == "San Mateo County, California"|
           NAME.x == "Sonoma County, California")

#Bay Area: Median House Value Visualization
bayarea %>%
  st_as_sf() %>%
  ggplot() +
  geom_sf(aes(fill = housingE)) +
