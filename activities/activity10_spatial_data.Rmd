...
ca_tracts <- ca_tracts %>%
  ...
  rename(medincome = medincomeE, fb = fbE, totp = totpE) %>%
  mutate(pfb = 100 * (fb / totp),
         p_black = 100 * (black_popE / totp),
         p_white = 100 * (white_popE / totp),
         p_asian = 100 * (asian_popE / totp),
         p_hispanic = 100 * (hisp_popE / totp)
         ) #%>%
  # mutate(medincome=ifelse(!is.na(medincome), medincome, 0))

tm_shape(ca_tracts %>%
           filter(GEOID != '06083990000')) +
  tm_polygons(col = "medincome", style = "quantile", palette = "BuGn", title = "Median Household Income") +
  tm_layout(main.title = "Median Household Income in Santa Barbara County, 2018-2022",
            main.title.position = "center",
            legend.outside = TRUE)
